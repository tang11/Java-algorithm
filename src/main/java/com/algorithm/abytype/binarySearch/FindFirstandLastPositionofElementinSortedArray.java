...
public class FindFirstandLastPositionofElementinSortedArray {
    ...
    public static int[] searchRange4(int[] nums, int target) {
        ...
        int left = findLeftIndex1(nums, target);
        if (left==-1) {
            return new int[]{-1,-1};
        }
        int right = findRightIndex1(nums,target,left);
        return new int[]{left,right};
    }
    ...
    public static int findRightIndex1(int nums[], int target, int start) {
        int left = start;
        int right = nums.length;
        while(left < right) {
            int mid = (left + right) /2;
...
//        System.out.println(Arrays.toString(searchRange3(nums ,1)));
        ...
        System.out.println(Arrays.toString(searchRange4(nums1 ,8)));
